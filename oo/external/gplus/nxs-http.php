...
File Version: 1.0.5 (April 2, 2014)
...
if (!class_exists('nxs_Http')){ class nxs_Http { private $headers = '';
    ...
    function curl_request($url, $args = array()) {
        ...
        if ( !empty( $r['headers'] ) ) { $headers = array(); foreach ( $r['headers'] as $name => $value )  $headers[] = "{$name}: $value"; curl_setopt( $handle, CURLOPT_HTTPHEADER, $headers );}
